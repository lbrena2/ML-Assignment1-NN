...
def verify(images, targets):
    ...
    max_index = max_index == targets
    num_ok = np.sum(max_index)
...
def validate():
    ...
    for i in range(0, len(validation_indices), batch_size):
        images = train_validation_set.images[validation_indices[i:batch_size + i]]
        labels = train_validation_set.labels[validation_indices[i:batch_size + i]]
...
for epoch in range(1000):
    ...
    for i in range(0, len(train_indices), batch_size):
        images = train_validation_set.images[train_indices[i:batch_size + i]]
        labels = train_validation_set.labels[train_indices[i:batch_size + i]]
        error += lib.train_one_step(model, loss, learning_rate, images, labels)
        batch_counter += 1
    error = error / float(batch_counter)
